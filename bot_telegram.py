...
def newgame_handler(bot, update, chat_data):
    ...
    if update.message.chat.type == "private":
        ...
        for waiting_player in waiting_players_per_group[chat_id]:
            bot.send_message(chat_id=waiting_player, text="A new game is starting in [{}](t.me/{})!".format(update.message.chat.title, chat_id), parse_mode=telegram.ParseMode.MARKDOWN)
...
def nextgame_handler(bot, update, chat_data):
    ...
    if game is not None and game.game_state == secret_hitler.GameStates.ACCEPT_PLAYERS and game.num_players<10 and update.message.text.find("confirm")==-1:
        ...
        bot.send_message(chat_id=update.message.from_user.id, text="I will notify you when a new game starts in [{}](t.me/{})".format(update.message.chat.title, chat_id), parse_mode=telegram.ParseMode.MARKDOWN)
    ...
    if player is None or player.game is None:
        ...
        if game is not None and game.game_state==secret_hitler.GameStates.ACCEPT_PLAYERS and game.num_players==9:
            for waiting_player in waiting_players_per_group[game.global_chat]:
                bot.send_message(chat_id=waiting_player, text="A slot just opened up in [{}](t.me/{})!".format(bot.get_chat(chat_id=game.global_chat).title, game.global_chat), parse_mode=telegram.ParseMode.MARKDOWN)
    if player is None:
