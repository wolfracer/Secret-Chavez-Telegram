    ...
    if update.message.chat.type == "private":
        ...
        bot.send_message(chat_id=chat_id,
                         text="A restart has been scheduled. No new games can be created while we wait for the remaining {} to finish.".format(
                             "game" if len(existing_games) == 1 else "{} games".format(len(existing_games))))
        ...
        asyncio.ensure_future(cancel_game_if_not_started_after_30_mins(bot, update, chat_data))
...
def nextgame_handler(bot, update, chat_data):
    ...
    if game is not None and game.game_state == secret_hitler.GameStates.ACCEPT_PLAYERS and game.num_players < 10 and update.message.text.find(
            "confirm") == -1:
        bot.send_message(chat_id=chat_id,
...
def restart_handler(bot, update):
    ...
    if chat_id == DEV_CHAT_ID and user_id in admin_ids:
        if len([game for game in existing_games if "{}".format(game) in existing_games and existing_games[
            "{}".format(game)].game_state != secret_hitler.GameStates.GAME_OVER]) > 0 and update.message.text.find(
                'confirm') == -1:
            bot.send_message(chat_id=chat_id,
            ...
            for game_chat_id in [int(game) for game in existing_games if
                                 "{}".format(game) in existing_games and existing_games[
